    ...
    "When using portfinder module": {
      "with 5 existing servers": {
        ...
        "the getPort() method": {
          ...
          "should respond with the first free port (32773)": function (
            err,
            port
          ) {
    ...
    "When using portfinder module": {
      "with 5 existing servers": {
        ...
        "the getPort() method with stopPort greater than available port": {
          ...
          "should respond with the first free port (32773) less than provided stopPort":
            function (err, port) {
              closeServers(); // close all the servers first!
    ...
    "When using portfinder module": {
      "with no existing servers": {
        ...
        "the getPort() method": {
          ...
          "should respond with the first free port (32768)": function (
            err,
            port
          ) {
            if (err) {
    ...
    "When using portfinder module": {
      "with no existing servers": {
        ...
        "the getPortPromise() method": {
          ...
          "should respond with a promise of first free port (32768) if Promise are available":
            function (err, port) {
      ...
      "with no available ports above the start port": {
        ...
        "the getPort() method requesting an unavailable port": {
          ...
          "should return error": function (err, port) {
            ...
            assert.isTrue(!!err);
            assert.equal(err.message, "No open ports available");
            return;
          },
        },
      },
