...
type MaybeErr = ErrnoException | undefined | null;
...
type GetSocketCallback = CallBack<string>;
type PortfinderCallback = CallBack<number>;
type GetPortsCallback = CallBack<(number | undefined)[]>;
...
const internals = {
  testPort: (_options: PortFinderOptions, callback: PortfinderCallback) => {
    ...
    if (!callback || typeof _options === "function") {
      ...
      options = _options as PortFinderOptions;
    }

    //
    // Create an empty listener for the port testing server.
    //
    options.server! ??= net.createServer(() => {});
...
export function getPort(
  _options: PortFinderOptions | PortfinderCallback,
  callback?: PortfinderCallback
) {
  let options: PortFinderOptions = {};
  if (!callback || typeof _options === "function") {
  ...
  if (!options.startPort) {
    ...
    if (options.startPort < 0) {
      throw Error(
        `Provided options.startPort(${options.startPort}) is less than 0, which are cannot be bound.`
      );
    }
    if (options.stopPort < options.startPort) {
      throw Error(
        `Provided options.stopPort(${options.stopPort}is less than options.startPort (${options.startPort})`
      );
    }
  }
...
export function getPortPromise(options: PortFinderOptions) {
  ...
  return new Promise((resolve, reject) => {
    const callback: PortfinderCallback = (err: MaybeErr, port?: number) => {
      if (err) {
        return reject(err);
      }
      resolve(port);
...
export function getPorts(
  ...
  _async.timesSeries<number, MaybeErr>(
    count,
    (index, asyncCallback) => {
      if (lastPort) {
      ...
      getPort(options, (err: MaybeErr, port?: number) => {
        if (err || port === undefined) {
          asyncCallback(err as ErrnoException);
        } else {
          lastPort = port;
...
export function getSocket(
  _options: GetSocketOptions | GetSocketCallback,
  callback?: GetSocketCallback
) {
  let options: GetSocketOptions = {};
  if (!callback || typeof _options === "function") {
    callback = _options as GetSocketCallback;
  } else {
    options = _options as GetSocketOptions;
  }

  options.mod! ??= parseInt("755", 8);
  options.path! ??= `${basePath}.sock`;
 ...
 */
export const _defaultHosts = ((): (string | null)[] => {
  ...
  const results: (string | null)[] = [hiddenButImportantHost];
  for (let i = 0; i < interfaceNames.length; i++) {
    const _interface = interfaces[interfaceNames[i]] as NetworkInterfaceInfo[];
