...
internals.testPort = function(options, callback) {
  ...
  function onListen () {
    ...
    client.on('data', function(data) {
      ...
      if (data === 'OK PORTFINDER') {
        ...
        options.server.emit('error', new Error(internals.bindError));
      }
    });
  ...
  function onError (err) {
    ...
    if (err.code !== 'EADDRINUSE' && err.code !== 'EACCES' && err.code !== internals.bindError.code) {
      return callback(err);
    }
 ...
 *         ]
 *
 *         Also note that we add 0.0.0.0 for ipv4 and '::' for ipv6 default
 *         hosts, which wont be return by os.networkInterfaces but are bound
 *         by each os to mean 'the default non-loopback interface'
 *
...
exports._defaultHosts = (function() {
  ...
  try{
    ...
    // the os.networkInterfaces() call and throws instead.
    //
