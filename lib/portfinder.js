...
exports.getPort = function (options, callback) {
  ...
  var hosts = [options.host, '127.0.0.1', '0.0.0.0'];

  // remove duplicate host arg if already in hosts array
  var uniqueHosts = hosts.filter(function(host, index) {
    return hosts.indexOf(host) == index;
  });

  var findCommonOpenPort = function(portToTest) {
    var ports = [];
    var error;
    async.every(uniqueHosts, function(host, asyncCallback) {
      ...
      if (result) {
        ...
        var uniquePorts = ports.filter(function(port, index) {
          return ports.indexOf(port) == index;
        });
        if (uniquePorts.length > 1) {
          // find highes port
          uniquePorts.sort(function(a, b) {
            return a - b;
          });
          // test all of the hosts against highest port
          findCommonOpenPort(uniquePorts.pop());
        } else {
  ...
  findCommonOpenPort(options.port || exports.basePort);
};
